...
class AvgData():
    "Class for calculating average data, wu_key and io_key, are for api keys for forecastio and wunderground"
    ...
    def temperature(self):
        ...
        wu_data = ScrapWu(self.get_lat_long()[0], self.get_lat_long()[1]
                          , self.offset, wu_key).get_temperature()
        io_data = ScrapIO(self.get_lat_long()[0], self.get_lat_long()[1]
                          , self.offset, io_key).get_avg_temperature()
        avg_temperature = int(wu_data + io_data)/2
        return avg_temperature

    def wind(self):
        'scraps wind from wunderground and forecast io and returns average daily in KpH'
        wu_data = ScrapWu(self.get_lat_long()[0], self.get_lat_long()[1]
                          , self.offset, wu_key).get_wind()
        io_data = ScrapIO(self.get_lat_long()[0], self.get_lat_long()[1]
                          , self.offset, io_key).get_avg_wind()
